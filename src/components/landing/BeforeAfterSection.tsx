    ...
    <section className="py-16 md:py-24 bg-gradient-to-b from-white to-gray-50 overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-6 max-w-5xl mx-auto">
          ...
          <div 
            ref={leftRef}
            className={`flex-1 bg-white rounded-xl p-6 md:p-8 border border-red-100 shadow-sm relative overflow-hidden transition-all duration-700 transform ${
              leftInView ? 'translate-x-0 opacity-100' : '-translate-x-20 opacity-0'
            }`}
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-red-50 rounded-full blur-xl -z-10 opacity-60"></div>
            ...
            <ul className="space-y-4 mb-6">
              {[
                ...
                <li key={index} className="flex items-start gap-3">
                  <div className="mt-1 p-1 rounded-full bg-red-50 text-red-400 flex-shrink-0">
                    <item.icon className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-gray-700">{item.text}</p>
                    <p className="text-red-500 text-sm font-semibold">{item.time}</p>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <div className="bg-red-50 p-4 rounded-lg">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-red-600">6+ {t('hours')}</p>
                <p className="text-xs text-gray-500 mt-1">{t('perArticle')}</p>
              </div>
            </div>
            
            <div className="w-16 h-16 bg-red-50 rounded-full blur-xl absolute bottom-4 left-8 -z-10 opacity-60"></div>
          </div>
          ...
          <div 
            ref={rightRef}
            className={`flex-1 bg-white rounded-xl p-6 md:p-8 border border-green-100 shadow-sm relative overflow-hidden transition-all duration-700 transform ${
              rightInView ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'
            }`}
          >
            <div className="absolute top-0 left-0 w-32 h-32 bg-green-50 rounded-full blur-xl -z-10 opacity-60"></div>
            ...
            <ul className="space-y-4 mb-6">
              {[
                ...
                <li key={index} className="flex items-start gap-3">
                  <div className="mt-1 p-1 rounded-full bg-green-50 text-green-500 flex-shrink-0">
                    <item.icon className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-gray-700">{item.text}</p>
                    <p className="text-green-600 text-sm font-semibold">{item.time}</p>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              <div className="text-center">
                <p className="text-sm text-gray-600">{t('totalTimeSpent')}</p>
                <p className="text-2xl font-bold text-green-600">1 {t('hour')}</p>
                <p className="text-xs text-gray-500 mt-1">{t('perArticle')}</p>
              </div>
            </div>
            
            <div className="w-16 h-16 bg-green-50 rounded-full blur-xl absolute bottom-4 right-8 -z-10 opacity-60"></div>
          </div>
        </div>
